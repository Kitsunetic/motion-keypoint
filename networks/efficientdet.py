...
__all__ = ["EfficientDetModel", "postprocess", "display"]
...
def efficientnet(
    width_coefficient=None, depth_coefficient=None, dropout_rate=0.2, drop_connect_rate=0.2, image_size=None, num_classes=1000
):
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params):
        ...
        self._depthwise_conv = Conv2d(
            in_channels=oup, out_channels=oup, groups=oup, kernel_size=k, stride=s, bias=False  # groups makes it depthwise
        )
        ...
        for j in range(batch_size):
            ...
            if positive_indices.sum() > 0:
                ...
                regression_loss = torch.where(
                    torch.le(regression_diff, 1.0 / 9.0), 0.5 * 9.0 * torch.pow(regression_diff, 2), regression_diff - 0.5 / 9.0
                )
...
class BiFPN(nn.Module):
    ...
    def __init__(
        self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True, use_p8=False
    ):
        """
...
class EfficientDetInsideModel(nn.Module):
    """
    modified by Zylo117
    """

    def __init__(self, compound_coef, load_weights=False):
        super(EfficientDetInsideModel, self).__init__()
...
class EfficientDetBackbone(nn.Module):
    def __init__(self, num_classes=90, compound_coef=0, load_weights=False, **kwargs):
        ...
        self.backbone_net = EfficientDetInsideModel(self.backbone_compound_coef[compound_coef], load_weights)
...
class EfficientDet(nn.Module):
    def __init__(self, model_name, pretrained=True):
        ...
        assert model_name in EFFICIENTDET_CHECKPOINT_URLS, f"`model_name` must be one of {EFFICIENTDET_CHECKPOINT_URLS.keys()}"

        self.model = EfficientDetBackbone(90, EFFICIENTDET_MODEL_INDEX[model_name])

        if pretrained:
            from torch.hub import get_dir

            ckpt_path = Path(get_dir()) / "checkpoints" / f"{model_name}.pth"
            if not ckpt_path.is_file():
                url = EFFICIENTDET_CHECKPOINT_URLS[model_name]
                print("Download checkpoint from", url, "to", ckpt_path)
...
def postprocess(x, anchors, regression, classification, regressBoxes=None, clipBoxes=None, threshold=0.2, iou_threshold=0.2):
    regressBoxes = regressBoxes or BBoxTransform()
    clipBoxes = clipBoxes or ClipBoxes()
